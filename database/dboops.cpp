...
bool insertTopic(MYSQL* conn, const std::string& title, const std::string& url, int id, int repositoryid) {
    // Escape strings to prevent SQL injection
   
    // Prepare the query
    char query[600];
    snprintf(query, sizeof(query),
             "INSERT INTO topics VALUES (%d, '%s', '%s', %d)",
             id, title.c_str(), url.c_str(), repositoryid);
...
int main(){
    ...
    switch(choice) {
        ...
        case 3: {
            ...
            std::cout << "Enter URL: ";
            std::cin >> url;
            if (DBManager::insertTopic(ptrDbManager->conn, title, url, id, repositoryid)) {
